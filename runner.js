...
var $author$project$Main$initialRunner = function (flags) {
	return {
		angle: 0.0,
		angleSnapshot: 0.0,
...
var $author$project$Main$initialModel = function (flags) {
	var runner = $author$project$Main$initialRunner(flags);
	return $author$project$Main$resetTheme(
		{
			highScore: 0,
...
var $author$project$Main$sanitizePfpUrl = function (url) {
	return A2($elm$core$String$startsWith, 'data:', url) ? A3(
...
var $author$project$Main$init = function (flags) {
	var sanitizedFlags = _Utils_update(
		flags,
		{
			pfpUrl: A2($elm$core$Maybe$map, $author$project$Main$sanitizePfpUrl, flags.pfpUrl)
		});
	return _Utils_Tuple2(
		$author$project$Main$initialModel(sanitizedFlags),
...
var $author$project$Main$keyDecoder = function (state) {
	return A2(
		$elm$json$Json$Decode$map,
		$author$project$Main$Key(state),
...
var $author$project$Main$onTouch = _Platform_incomingPort('onTouch', $elm$json$Json$Decode$string);
...
var $author$project$Main$applyKey = F3(
	function (state, key, model) {
		return $author$project$Main$cmdless(
			function () {
				var oldInput = model.input;
				var keys = state ? A2($elm$core$Set$insert, key, oldInput.keys) : A2($elm$core$Set$remove, key, oldInput.keys);
				var input = _Utils_update(
					oldInput,
					{
						jump: A2($elm$core$Set$member, 'Up', keys) || (A2($elm$core$Set$member, 'ArrowUp', keys) || A2($elm$core$Set$member, 'RightTouch', keys)),
						keys: keys,
						slide: A2($elm$core$Set$member, 'Down', keys) || (A2($elm$core$Set$member, 'ArrowDown', keys) || A2($elm$core$Set$member, 'LeftTouch', keys))
					});
				return _Utils_update(
					model,
					{input: input});
	...
	return $author$project$Main$cmdless(
		($elm$core$Set$isEmpty(model.input.keys) || (model.millis < 1000)) ? model : A2(
			...
			$author$project$Main$resetTheme(
				_Utils_update(
					model,
					{runner: model.initialRunner, score: 0, track: $author$project$Main$initialTrack}))));
};
...
var $author$project$Main$setRunnerState = F3(
	function (input, state, runner) {
		return _Utils_update(
			runner,
			{
				angleSnapshot: (runner.angle < 360.0) ? runner.angle : 0.0,
...
var $author$project$Main$jumpStep = F2(
	function (input, runner) {
		...
		return (_Utils_cmp(d, ds) < 1) ? A3(
			...
			_Utils_update(
				runner,
				{angle: runner.angleSnapshot + 90.0, y: 0})) : _Utils_update(
			runner,
			{angle: runner.angleSnapshot + ((ds / d) * 90.0), y: (a * ds) * (d - ds)});
	});
...
var $author$project$Main$advanceObstacles = F2(
	function (input, track) {
		...
		return _Utils_update(
			track,
			{
				obstacles: A2($elm$core$List$map, aux, track.obstacles)
			});
	});
var $author$project$Main$canvasLeft = -$author$project$Main$runnerX;
...
var $author$project$Main$dropInvisibleObstacles = function (track) {
	var aux = function (obstacle) {
		return _Utils_cmp(obstacle.x + (obstacle.units * $author$project$Main$obstacleUnit), $author$project$Main$canvasLeft) < 1;
	};
	return _Utils_update(
		track,
		{
...
var $author$project$Main$Spikes = {$: 'Spikes'};
...
var $author$project$Main$maybeSpawnObstacle = F2(
	function (input, track) {
		return A2(
			$elm$core$Tuple$pair,
			track,
...
var $author$project$Main$trackStep = F2(
	function (input, track) {
		return A2(
			$author$project$Main$maybeSpawnObstacle,
			input,
			$author$project$Main$dropInvisibleObstacles(
...
var $author$project$Main$step = function (model) {
	var _v0 = model.state;
	if (_v0.$ === 'Paused') {
		return $author$project$Main$pausedStep(model);
	} else {
		return $author$project$Main$playingStep(model);
	}
};
...
var $author$project$Main$applyTouch = F2(
	function (touch, model0) {
		var _v0 = A3(
			$author$project$Main$applyKey,
			A2($elm$core$String$contains, 'L', touch),
			'LeftTouch',
			model0);
		var model1 = _v0.a;
		var cmd1 = _v0.b;
		var _v1 = A3(
			$author$project$Main$applyKey,
			A2($elm$core$String$contains, 'R', touch),
			'RightTouch',
			model1);
		var model2 = _v1.a;
		var cmd2 = _v1.b;
	...
	function (msg, model) {
		switch (msg.$) {
			case 'Key':
				var state = msg.a;
				var key = msg.b;
				return A3($author$project$Main$applyKey, state, key, model);
			case 'Touch':
				var touch = msg.a;
				return A2($author$project$Main$applyTouch, touch, model);
			case 'Tick':
				var posix = msg.a;
...
var $elm$svg$Svg$feImage = $elm$svg$Svg$trustedNode('feImage');
...
var $author$project$Main$rotate = function (angle) {
	return 'rotate(' + ($elm$core$String$fromFloat(angle) + ')');
};
...
var $author$project$Main$runnerView = function (runner) {
	return A2(
		$elm$svg$Svg$g,
		_List_fromArray(
			[
				$elm$svg$Svg$Attributes$transform(
				A2($author$project$Main$translate, $author$project$Main$runnerX, ($author$project$Main$canvasHeight - runner.y) - (runner.h / 2)) + (' ' + (A2($author$project$Main$scale, runner.w, runner.h) + (' ' + $author$project$Main$rotate(runner.angle)))))
			]),
		_Utils_ap(
			function () {
				var _v0 = runner.pfpUrl;
				if (_v0.$ === 'Nothing') {
					return _List_Nil;
				} else {
					var pfpUrl = _v0.a;
										...
										[
											...
											_List_fromArray(
												[
													...
													$elm$svg$Svg$Attributes$values('0.9 0   0   0   0.1 ' + ('0   0.9 0   0   0.1 ' + ('0   0   0.9 0   0.1 ' + '0   0   0   1   0'))),
													$elm$svg$Svg$Attributes$result('r1')
												]),
											_List_Nil),
											A2(
											$elm$svg$Svg$feBlend,
											_List_fromArray(
												[
													$elm$svg$Svg$Attributes$in_('r0'),
													$elm$svg$Svg$Attributes$in2('r1'),
													$elm$svg$Svg$Attributes$mode('multiply')
												]),
											_List_Nil)
										]))
								]))
...
var $author$project$Main$iter = F2(
	function (n, f) {
		var aux = F2(
			function (m, xs) {
				aux:
		...
		return A2(aux, n, _List_Nil);
	});
var $author$project$Main$points = function (ps) {
	return A2(
		$elm$core$String$join,
		' ',
		A2(
...
var $author$project$Main$obstacleView = function (obstacle) {
	var _v0 = obstacle.kind;
	if (_v0.$ === 'Spikes') {
		var spikeView = function (i) {
			var x0 = i * $author$project$Main$obstacleUnit;
			return A2(
				$elm$svg$Svg$polygon,
				_List_fromArray(
					[
						$elm$svg$Svg$Attributes$points(
						$author$project$Main$points(
							_List_fromArray(
								[
									_Utils_Tuple2(x0, 0),
									_Utils_Tuple2(x0 + ($author$project$Main$obstacleUnit / 2), -$author$project$Main$spikeH),
									_Utils_Tuple2(x0 + $author$project$Main$obstacleUnit, 0)
								])))
					]),
				_List_Nil);
		};
		return A2(
			...
			_List_fromArray(
				[
					$elm$svg$Svg$Attributes$transform(
					A2($author$project$Main$translate, $author$project$Main$runnerX + obstacle.x, $author$project$Main$canvasHeight))
				]),
			A2($author$project$Main$iter, obstacle.units, spikeView));
	} else {
		return A2(
			$elm$svg$Svg$rect,
			_List_fromArray(
				[
					...
					$elm$core$String$fromFloat($author$project$Main$runnerX + obstacle.x)),
...
var $author$project$Main$trackView = function (track) {
	return A2(
		$elm$svg$Svg$g,
		_List_Nil,
		A2($elm$core$List$map, $author$project$Main$obstacleView, track.obstacles));
};
var $author$project$Main$playingView = function (model) {
	return _List_fromArray(
		[
			$author$project$Main$runnerView(model.runner),
