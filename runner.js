...
Elm.Main.make = function (_elm) {
   ...
   var Hole = {ctor: "Hole"};
   var Spikes = {ctor: "Spikes"};
   var obstacleKindGenerator = function () {
   ...
   var Sliding = {ctor: "Sliding"};
   var Jumping = {ctor: "Jumping"};
   ...
   var Jump = {ctor: "Jump"};
   var setIsEmpty = function (xs) {
      return _U.eq(A3($Set.foldl,
      F2(function (_v3,s) {
         return function () {
            return s + 1;
         }();
   ...
   var resolveKeys = function (keysDown) {
      return function () {
         var _v5 = {ctor: "_Tuple2"
                   ,_0: A2($Set.member,38,keysDown)
                   ,_1: A2($Set.member,
                   40,
                   keysDown)};
         switch (_v5.ctor)
         {case "_Tuple2": switch (_v5._0)
              {case false: switch (_v5._1)
                   {case true: return Slide;}
                   break;
                 case true: switch (_v5._1)
                   {case false: return Jump;}
                   break;}
              break;}
         return setIsEmpty(keysDown) ? None : Some;
      }();
   };
   var key = A2($Signal.map,
   resolveKeys,
   $Keyboard.keysDown);
   var input = A2($Signal._op["~"],
   A2($Signal._op["<~"],Input,key),
   time);
   ...
   var lerp = F3(function (x,
   y,
   alpha) {
      return (1 - alpha) * x + alpha * y;
   });
   var dimTransition = F5(function (input,
   duration,
   w,
   h,
   runner) {
      return function () {
         var $ = runner.dimSnapshot,
         w0 = $._0,
         h0 = $._1;
         var dt = input.time - runner.snapshotTime;
         var alpha = A2($Basics.min,
         duration,
         dt) / duration;
         return _U.replace([["w"
                            ,A3(lerp,w0,w,alpha)]
                           ,["h",A3(lerp,h0,h,alpha)]],
         runner);
      }();
   });
   ...
   runner) {
      return function () {
         ...
         runner) : A4(dimTransition,
         input,
         dimTransitionDuration,
         runnerSize,
         runnerSize)(_U.replace([["y"
                                 ,a * dt * (d - dt)]
                                ,["angle"
                                 ,(0 - dt) / d * $Basics.pi / 2]],
         runner));
   ...
   var slideStep = function (input) {
      return A4(dimTransition,
      input,
      dimTransitionDuration,
      2 * runnerSize,
      runnerSize / 2);
   };
   var runStep = function (input) {
      return A4(dimTransition,
      input,
      dimTransitionDuration,
      runnerSize,
   ...
   var runnerStep = F2(function (input,
   runner) {
      return function () {
         ...
         return _U.eq(input.key,
         Jump) && !_U.eq(runner.state,
         Jumping) ? setState(Jumping) : _U.eq(input.key,
         Slide) && _U.eq(runner.state,
         Running) ? setState(Sliding) : _U.eq(input.key,
         None) && _U.eq(runner.state,
         Sliding) ? setState(Running) : _U.eq(runner.state,
         Jumping) ? A2(jumpStep,
         input,
         runner) : _U.eq(runner.state,
         Sliding) ? A2(slideStep,
         input,
         runner) : A2(runStep,
         input,
         runner);
   ...
   var collides = F2(function (runner,
   obstacle) {
      return function () {
         var h = runner.h;
         var y = runner.y;
         var x = 0;
         var x1 = obstacle.x + $Basics.toFloat(obstacle.units) * obstacleUnit + runner.w / 2 - obstacleMargin;
         var x0 = obstacle.x - runner.w / 2 + obstacleMargin;
         return function () {
            var _v14 = obstacle.kind;
            switch (_v14.ctor)
            {case "Hole": return _U.cmp(x0,
                 0) < 0 && (_U.cmp(0,
                 x1) < 0 && _U.cmp(h,
                 runnerSize / 2) > 0);
               case "Spikes": return _U.cmp(y,
                 spikeH - obstacleMargin) < 0 && (_U.cmp(y,
   ...
   var collisions = F2(function (input,
   model) {
      return A2($List.any,
   ...
   var fg = $Color.white;
   var runnerForm = function (runner) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: 0
                                    ,_1: runner.y + runner.h / 2})($Graphics$Collage.rotate(runner.angle)($Graphics$Collage.filled(fg)(A2($Graphics$Collage.rect,
      runner.w,
      runner.h))));
   };
   ...
   var obstacleForm = function (obstacle) {
      return function () {
         var _v15 = obstacle.kind;
         switch (_v15.ctor)
         {case "Hole":
            return function () {
                 var h = canvasHeight - runnerSize / 2;
                 var w = $Basics.toFloat(obstacle.units) * obstacleUnit;
                 return $Graphics$Collage.move({ctor: "_Tuple2"
                                               ,_0: obstacle.x + w / 2
                                               ,_1: h / 2 + runnerSize / 2})($Graphics$Collage.filled(fg)(A2($Graphics$Collage.rect,
                 w,
                 h)));
              }();
            case "Spikes":
            return function () {
                 var radius = 2 * spikeH / 3;
                 var spikeForm = function (i) {
                    return $Graphics$Collage.move({ctor: "_Tuple2"
                                                  ,_0: $Basics.toFloat(i) * obstacleUnit
                                                  ,_1: 0})($Graphics$Collage.rotate($Basics.pi / 2)($Graphics$Collage.filled(fg)(A2($Graphics$Collage.ngon,
                    3,
                    radius))));
                 };
                 return $Graphics$Collage.move({ctor: "_Tuple2"
                                               ,_0: obstacle.x + obstacleUnit / 2
                                               ,_1: spikeH / 3})($Graphics$Collage.group(A2(iter,
                 obstacle.units,
                 spikeForm)));
              }();}
         _U.badCase($moduleName,
   ...
   var playingForm = function (model) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: runnerX
                                    ,_1: (0 - canvasHeight) / 2})($Graphics$Collage.group(_L.fromArray([runnerForm(model.runner)
                                                                                                       ,trackForm(model.track)])));
   };
   var canvasWidth = 600;
   var canvasLeft = (0 - canvasWidth) / 2 - runnerX;
   var dropInvisibleObstacles = function (track) {
      return function () {
         var aux = function (obstacle) {
            return _U.cmp(obstacle.x + $Basics.toFloat(obstacle.units) * obstacleUnit,
            canvasLeft) < 1;
         };
   ...
   var canvasRight = canvasWidth / 2 - runnerX;
   var advanceObstacles = F2(function (input,
   track) {
      return function () {
         var x0 = canvasRight;
         var aux = function (obstacle) {
            return _U.replace([["x"
                               ,x0 - (input.time - obstacle.spawnTime) * speed]],
            obstacle);
         };
         return _U.replace([["obstacles"
                            ,A2($List.map,
   ...
   var maybeSpawnObstacle = F2(function (input,
   track) {
      return function () {
         var xMax = function () {
            var _v16 = last(track.obstacles);
            switch (_v16.ctor)
            {case "Just":
         ...
         minObstacleDistance) > -1 ? function () {
            ...
            seed1 = $._1;
            var t = input.time;
            var spawnTimeGenerator = A2($Random.$float,
            t,
            t + minObstacleDistance / speed);
            var $ = A2($Random.generate,
            spawnTimeGenerator,
            seed1),
            spawnTime = $._0,
            ...
            return _U.replace([["obstacles"
                               ,A2($Basics._op["++"],
                               track.obstacles,
                               _L.fromArray([obstacle]))]
                              ,["seed",seed3]],
            track);
         }() : track;
      }();
   });
   var trackStep = F2(function (input,
   track) {
      return maybeSpawnObstacle(input)(dropInvisibleObstacles(advanceObstacles(input)(track)));
   });
   var playingStep = F2(function (input,
   model) {
      return collisions(input)(_U.replace([["runner"
                                           ,A2(runnerStep,
                                           input,
                                           model.runner)]
                                          ,["track"
      ...
      return function () {
         ...
         {case "_Tuple2":
            return A4($Graphics$Element.container,
              ...
              canvasWidth,
              canvasHeight,
              _L.fromArray([$Graphics$Collage.filled(bg)(A2($Graphics$Collage.rect,
                           canvasWidth,
                           canvasHeight))
                           ,function () {
   ...
   _elm.Main.values = {_op: _op
                      ...
                      ,dimTransitionDuration: dimTransitionDuration
                      ,speed: speed
                      ...
                      ,obstacleKindGenerator: obstacleKindGenerator
                      ,advanceObstacles: advanceObstacles
                      ,dropInvisibleObstacles: dropInvisibleObstacles
                      ,maybeSpawnObstacle: maybeSpawnObstacle
                      ,trackStep: trackStep
                      ,Paused: Paused
                      ,Playing: Playing
                      ,Model: Model
